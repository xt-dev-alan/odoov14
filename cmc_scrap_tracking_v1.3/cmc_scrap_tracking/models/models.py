...
class cmc_inventory_report(models.Model):
    ...
    def _compute_net(self):
        for record in self:
            # Due to the race condition it must be verified if inventory_quantity is zero(this is because the
            # internal odoo process to update the model stock.quant with data from the lot model)
            gross_weight = record.inventory_quantity if record.inventory_quantity != 0 else record.cmc_lot_gross_weight
            record.cmc_lot_net_weight = gross_weight - record.cmc_lot_tare_weight
